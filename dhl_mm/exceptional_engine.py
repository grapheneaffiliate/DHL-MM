...
class ExceptionalAlgebra:
    ...
    @classmethod
    def from_cache(cls, name: str, data: dict) -> "ExceptionalAlgebra":
        """Create an ExceptionalAlgebra from preloaded numpy arrays.

        Args:
            name: Algebra name (G2, F4, E6, E7, E8)
            data: dict with keys 'fI', 'fJ', 'fK', 'fC', 'killing', 'dim', 'rank', 'n_roots'

        Returns:
            ExceptionalAlgebra instance without recomputing structure constants.
        """
        obj = cls.__new__(cls)
        obj.name = name
        obj._dim = int(data['dim'])
        obj._rank = int(data['rank'])
        obj._n_roots = int(data['n_roots'])
        obj.fI = data['fI'].astype(np.int32)
        obj.fJ = data['fJ'].astype(np.int32)
        obj.fK = data['fK'].astype(np.int32)
        obj.fC = data['fC'].astype(np.float64)
        obj.killing = data['killing'].astype(np.float64)
        # Build adjoint lazily only when needed
        obj._gen_array = None
        obj.roots = None
        obj.simple_roots_arr = None
        obj.cartan_mat = None
        obj._d_verified = False
        obj._d_max = None
        obj._d_vanishes = not has_cubic_casimir(name)
        return obj

    @property
    def gen_array(self):
        """Adjoint representation matrices (built lazily)."""
        if self._gen_array is None:
            self._gen_array = self._build_adjoint()
        return self._gen_array

    @gen_array.setter
    def gen_array(self, value):
        self._gen_array = value

    @property
    def algebra_name(self) -> str:
        """Alias for name, for API compatibility."""
        return self.name
